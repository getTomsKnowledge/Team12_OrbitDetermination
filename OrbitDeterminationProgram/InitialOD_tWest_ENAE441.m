...
[R1Final.position_m, R2Final.position_m, R3Final.position_m] = gauss_solver(...
    obs1ECI.position_m, obs2ECI.position_m, obs3ECI.position_m, ...
    chile1.position_m, chile2.position_m, chile3.position_m, ...
    t1, t2, t3);

try
    [V1Final, V2Final, V3Final] = gibbs_solver(...
        R1Final.position_m, R2Final.position_m, R3Final.position_m, ...
        consts.mu_earth_km*1e9);
catch ME
    errorText = getReport(ME, "extended");
    warning('Invalid orbit');
end

% Create histogram of data:
figure('Name', 'Histogram of Orbit Combinatoric RMS values');
histogram(rmsDistribution);
title('Histogram of RMS Values for Randomly-Selected Observations, Combinatoric Tuples');

xIn1 = [R1Final.position_m; V1Final];
xIn2 = [R2Final.position_m; V2Final];
xIn3 = [R3Final.position_m; V3Final];
tRange = [0 90*60];


[tOut, orbit1Final] = ode45(@propagate_2BP, tRange, xIn1, [], consts.mu_earth_km*1e9);
[tOut, orbit2Final] = ode45(@propagate_2BP, tRange, xIn2, [], consts.mu_earth_km*1e9);
[tOut, orbit3Final] = ode45(@propagate_2BP, tRange, xIn3, [], consts.mu_earth_km*1e9);

figure('Name', 'Initial Orbit Plot');
plot3(orbit1Final(:,1), orbit1Final(:,2), orbit1Final(:,3), 'Color', 'r');
hold on;
plot3(orbit2Final(:,1), orbit2Final(:,2), orbit2Final(:,3), 'Color', 'g');
hold on;
plot3(orbit3Final(:,1), orbit3Final(:,2), orbit3Final(:,3), 'Color', 'b');


[hOut, pOut, ksStat, cVal] = kstest(rmsDistribution, 'unequal');

%{
try 
    kstest(rmsDistribution, 'unequal');
catch ME
    msgText = getReport(ME, "basic");
    fprintf('Did some shit instead.\n\n');
end
%}

if (hOut)
    fprintf('\n\n     By Kolmogorov-Smirnov Test:  Distribution is *NOT* a normal distribution:\n');
    fprintf('     C = %0.4f\n\n', cVal)
end
